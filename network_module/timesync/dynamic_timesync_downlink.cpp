...
namespace mxnet{
DynamicTimesyncDownlink::~DynamicTimesyncDownlink() {
}
...
void DynamicTimesyncDownlink::periodicSync(long long slotStart) {
    ...
    for (; !(success || rcvResult.error == RecvResult::ErrorCode::TIMEOUT);
            success = isSyncPacket(rcvResult, packet, networkConfig->panId, ctx.getHop())) {
        try {
            //uncorrected TS needed for computing the correction with flopsync
            rcvResult = transceiver.recv(packet, syncPacketSize, timeoutTime, Transceiver::Unit::NS, Transceiver::Correct::UNCORR);
        ...
        if (ENABLE_PKT_INFO_DBG) {
            if(rcvResult.size){
                print_dbg("[T] err=%d D=%d", rcvResult.error, result.size);
    ...
    if (success) {
        ...
        syncStatus->receivedPacket(rcvResult.timestamp);
        auto correctedMeasuredTimestamp = syncStatus->correct(rcvResult.timestamp);
        rebroadcast(correctedMeasuredTimestamp);
        transceiver.turnOff();
        measuredGlobalFirstActivityTime = correctedMeasuredTimestamp - packet[2] * rebroadcastInterval;
        if (ENABLE_TIMESYNC_DL_INFO_DBG)
...
void DynamicTimesyncDownlink::resync() {
    ...
    for (bool success = false; !success; success = isSyncPacket(rcvResult, packet, networkConfig->panId)) {
        try {
            rcvResult = transceiver.recv(packet, syncPacketSize, infiniteTimeout);
        } catch(std::exception& e) {
            if (ENABLE_RADIO_EXCEPTION_DBG)
                print_dbg("%s\n", e.what());
        }
    ...
    auto measuredGlobalFirstActivityTime = rcvResult.timestamp - packet[2] * rebroadcastInterval;
    ++packet[2];
    syncStatus->initialize(rcvResult.timestamp);
    ctx.setHop(packet[2]);
    rebroadcast(syncStatus->correct(rcvResult.timestamp), packet, networkConfig->maxHops);

    ledOff();
    transceiver.turnOff();

    if (ENABLE_TIMESYNC_DL_INFO_DBG)
        print_dbg("[F] hop=%d ats=%lld ets=%lld w=%d mts=%lld rssi=%d\n",
                packet[2], rcvResult.timestamp, status->computedFrameStart, status->receiverWindow,
                measuredGlobalFirstActivityTime, rcvResult.rssi
        );

    if (packet[2] == 1)
        ctx.getTopologyContext()->addNeighbor(0);
    else
        ctx.getTopologyContext()->removeNeighbor(0);
}
...
inline void DynamicTimesyncDownlink::execute(long long slotStart)
{
    ...
    if (false && ctx.getTopologyContext()->hasSuccessor(0))
